    ...
    if (cartItems.length === 0) {
        ...
        cartItems.forEach(item => {
            ...
            if (!item.isBeverage) {
                ...
                toggleBtn.innerHTML = `
                    <span>⚙️ Personalizar (Ensalada, Papas, Cremas y Extras)</span>
                ...
                // 2. Selector de Tipo de Papa (Fritas / Al Hilo / Mixta)
...
export function showConfirmOrderModal(customer, items, totals) {
    ...
    const itemsRows = items.map(it => {
        ...
        if (!it.isBeverage) {
            ...
            const papaBadge = papaMap[it.tipoPapa] || '🍟 Papas fritas';
...
export function printThermalTicket(info) {
    ...
    items.forEach(it => {
        ...
        if (!it.isBeverage) {
            ...
            const papaMap = { 'fritas': 'PAPAS FRITAS', 'hilo': 'PAPAS AL HILO', 'mixta': 'PAPAS MIXTAS' };
            const papaText = papaMap[it.tipoPapa] || 'PAPAS FRITAS';
