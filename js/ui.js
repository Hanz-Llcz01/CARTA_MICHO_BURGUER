...
export function updateCartUI(cartItems, totals, onUpdateQty, onCustomAction) {
    ...
    if (cartItems.length === 0) {
        ...
        cartItems.forEach(item => {
            ...
            minusBtn.title = (!item.isBeverage || item.qty === 1) ? 'Eliminar este producto' : 'Reducir cantidad';
            ...
            plusBtn.textContent = '+';
            plusBtn.title = item.isBeverage ? 'Aumentar cantidad' : 'Duplicar este plato para personalizarlo';
            ...
            if (!item.isBeverage) {
                ...
                const papaPart = item.allowsPotatoChoice ? ` | 🥔 <strong>Papa:</strong> ${papaMap[item.tipoPapa] || 'Papas Fritas'}` : '';
                ...
                summaryEl.innerHTML = `🥗 <strong>${ensaladaLabel}</strong>${papaPart} | 🥫 <strong>Cremas:</strong> ${cremasPedStr} | 🥡 <strong>Aparte:</strong> ${cremasApartStr}${extrasList}${notaStr}`;
                ...
                // 4. Cremas (dentro del sándwich o plato)
                ...
                cremasPedidoBlock.innerHTML = `
                    <div class="cremas-sub-header">
                        <span class="cremas-sub-title">🥫 Cremas:</span>
        ...
        if (!it.isBeverage) {
            ...
            specsHtml = `
                <div class="summary-item-specs">
                    ...
                    <div class="sub-spec">🥫 <strong>Cremas:</strong> ${pedCremas}</div>
                    ${aparteCremas}
                    ${extrasText}
                    ${notesText}
                </div>
