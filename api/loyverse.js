...
export default async function handler(req, res) {
  ...
  // Detalle para nota de comanda
  let deliveryDetails = '';
  if (orderType === 'A Enviar') {
    deliveryDetails = ` | Cel: ${phone} | Dir: ${address}${building ? ' (' + building + ')' : ''}`;
  }

  const itemsSpecs = items.map(it => {
    let specParts = [];
    if (!it.isBeverage) {
      specParts.push(`Ens: ${it.ensalada === 'no' ? 'NO' : 'SÍ'}`);
      const papaLabel = it.tipoPapa === 'hilo' ? 'Hilo' : (it.tipoPapa === 'mixta' ? 'Mixta' : 'Fritas');
      specParts.push(`Papa: ${papaLabel}`);
      if (it.cremasEnPedido && it.cremasEnPedido.length > 0) {
        specParts.push(`En ped: ${it.cremasEnPedido.join(',')}`);
      }
      if (it.cremasAparte && it.cremasAparte.length > 0) {
        specParts.push(`Aparte: ${it.cremasAparte.join(',')}`);
      }
      if (it.extras && it.extras.length > 0) {
        specParts.push(`+${it.extras.map(e => e.name).join(',')}`);
      }
      if (it.instrucciones && it.instrucciones.trim()) {
        specParts.push(`Det: ${it.instrucciones.trim()}`);
      }
    }
    const spec = specParts.length > 0 ? ` [${specParts.join(' | ')}]` : '';
    return `${it.qty}x ${it.name}${spec}`;
  }).join(' ; ');

  const noteDetails = `COMANDA WEB | ${orderType.toUpperCase()} | ${clientName}${deliveryDetails} | Pago: ${paymentLabel} | Platos: ${itemsSpecs} | Notas: ${notes}`;
