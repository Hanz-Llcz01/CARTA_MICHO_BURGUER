...
export default async function handler(req, res) {
  ...
  const itemsSpecs = items.map(it => {
    let spec = '';
    if (it.cremas && it.cremas.length > 0) {
      spec += ` [Cremas: ${it.cremas.join(',')} (${it.cremasMode === 'aparte' ? 'APARTE' : 'EN PEDIDO'})]`;
    }
    if (it.extras && it.extras.length > 0) {
      spec += ` [+${it.extras.map(e => e.name).join(',')}]`;
    }
      ...
      const itemsList = items.map(it => {
        ...
        if (!it.isBeverage) {
          const cremasText = (it.cremas && it.cremas.length > 0)
            ? `\n    🥫 _Cremas:_ ${it.cremas.join(', ')} *(${it.cremasMode === 'aparte' ? 'APARTE' : 'EN EL PEDIDO'})*`
            : `\n    🥫 _Cremas:_ _Sin cremas_`;
          const extrasText = (it.extras && it.extras.length > 0)
            ? `\n    🧀 _Adicionales:_ ${it.extras.map(e => `${e.name} (+S/ ${Number(e.price).toFixed(2)})`).join(', ')}`
            : '';
          details = `${cremasText}${extrasText}`;
        }
